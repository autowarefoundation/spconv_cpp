...
tv::Tensor SpconvOps::sort_1d_by_key_split_allocator(tv::Tensor data, std::function<std::uintptr_t(std::size_t)> alloc_func, tv::Tensor mask, tv::Tensor indices, std::uintptr_t stream, bool mask_output)   {
  ...
  tv::dispatch<int32_t, uint32_t, int64_t, uint64_t>(data.dtype(), [&](auto I){
      ...
      thrust_replacement::sort_by_key(ctx2, ptr_tr, ptr_tr + data.dim(0), ptr_k, op_comp);
      if (mask_output){
